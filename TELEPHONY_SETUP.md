# NTC WebCall Telephony Setup

NTC WebCall exposes inbound phone webhooks for both Twilio and Telnyx.

## Environment

Set these on the NTC WebCall server:

- `NTC_TELEPHONY_SECRET`
- `NTC_TWILIO_WEBHOOK_TOKEN`
- `NTC_TELNYX_WEBHOOK_TOKEN`
...
2. Set the incoming voice webhook URL to:
   ...
   `https://ntcnas.myftp.org/webcall/telephony/twilio/<NTC_TWILIO_WEBHOOK_TOKEN>/voice`
...
- NTC WebCall resolves the active room for that PIN
...
3. Set the TeXML application URL to:
   ...
   `https://ntcnas.myftp.org/webcall/telephony/telnyx/<NTC_TELNYX_WEBHOOK_TOKEN>/voice`
